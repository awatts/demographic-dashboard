...
all_subjects <- bind_rows(crossling, mturk) %>% select(-record_id, -workerid)
...
overall_rate <- nrow(crossling) + nrow(mturk) + mexico_count
...
mturk_rate <- nrow(mturk)
gauge(mturk_rate, min = 0, max = mturk_max, abbreviate = FALSE,
      gaugeSectors(
